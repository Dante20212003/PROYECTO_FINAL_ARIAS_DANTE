...
namespace PROYECTO_FINAL_ARIAS_DANTE.Vistas.Ventas
{
    public partial class FormularioVenta_v2 : Form
    {
        ...
        public static string peliculaSelect;
        public static string clienteSelect;

        public FormularioVenta_v2(string id = "")
        {
            this.id = id;
            InitializeComponent();
        }
            ...
            if (id.Length > 0)
            {
                ...
                foreach (Venta item in FormVentas.ListaVentas.getVentas())
                {
                    if (item.IdVenta.ToString() == id)
                    {
                        ...
                        inpPelicula.Text = item.Pelicula;
                        inpPelicula.Enabled = false;

                        inpCliente.Text = item.Cliente;
                        inpCliente.Enabled = false;
        ...
        private void btnGenerarVenta_Click(object sender, EventArgs e)
        {
            ...
            if (check)
            {
                ...
                venta.Pelicula = inpPelicula.Text;
                venta.Cliente = inpCliente.Text;
        ...
        {
            FormPeliculas fr = new FormPeliculas(true);
            fr.Size = new Size(890, 523);

            MenuPrincipal.ShowMessage("Doble Click para añadir una pelicula", "Alerta", "info");

            if (fr.ShowDialog() == DialogResult.OK)
            {
                inpPelicula.Text = peliculaSelect;
            };
                validatePelicula();
        }
        ...
        {
            FormClientes fr = new FormClientes(true);
            fr.Size = new Size(750, 500);

            MenuPrincipal.ShowMessage("Doble Click para añadir un cliente", "Alerta", "info");

            if (fr.ShowDialog() == DialogResult.OK)
            {
                inpCliente.Text = clienteSelect;
            }
        ...
        void print()
        {
            ...
            {
                MenuPrincipal.ShowMessage($"Error con la impresion. Verifique lo siguiente\n\n-Habilitar el servicio \"Cola de Impresion\" de Windows\n\n{ex.Message}", "Error", "error");
            }
        ...
        {
            Pelicula pelicula = ListaPeliculas.getPelicula(inpPelicula.Text);
            Cliente cliente = ListaClientes.getCliente(inpCliente.Text);
            Empresa DataEmpresa = MenuPrincipal.EmpresaData.getEmpresa();

            e.Graphics.DrawString("GRACIAS POR SU COMPRA", new Font("Arial", 20, FontStyle.Bold), Brushes.Black, new Point(250, 40));

            e.Graphics.DrawString($"Empresa:   {DataEmpresa.Nombre}", new Font("Arial", 16, FontStyle.Regular), Brushes.Black, new Point(100, 90 ));

            e.Graphics.DrawString($"Nit:   {DataEmpresa.Nit}", new Font("Arial", 16, FontStyle.Regular), Brushes.Black, new Point(100, 120));

            e.Graphics.DrawString($"Fecha Compra:   {datePicker.Value}", new Font("Arial", 16, FontStyle.Regular), Brushes.Black, new Point(100, 90 + 60));
            ...
            e.Graphics.DrawString($"Pelicula:   {inpPelicula.Text}", new Font("Arial", 16, FontStyle.Regular), Brushes.Black, new Point(100, 170 + 60));
